...
Plug 'fatih/vim-go', {'branch': 'master'}                                " Go completion and features
...
let g:coc_global_extensions = [ 'coc-json', 'coc-tsserver', 'coc-go', 'coc-flutter', 'coc-css', 'coc-prettier' , 'coc-clangd']
...
lua require('jump_to_proto')

function! SetupPbGoMapping()
  if expand('%:e') == 'go' && expand('%:t') =~ '\.pb\.go$'
    nnoremap <buffer> gd :lua require('jump_to_proto').jump_to_proto()<CR>
  endif
endfunction

autocmd FileType go call SetupPbGoMapping()
...
let g:goimports_cmd = '/home/pallav/.GO/bin/goimports'
